...
func PBEBGossiper2(port int, round *int, isGossipList, changePList, pullResponseList map[int]bool, pList, colored map[int]int, ch chan int, csvWriter *csv.Writer) {
	...
	for {
		...
		go func(msg Message) {
			...
			if msg.Data == "request update" {
				response = true
				lockForPullResponseList.Lock()
				pullResponseList[port] = true
				lockForPullResponseList.Unlock()
				requestAddr = *addr
				return
			}
			lockForColored.Lock()
