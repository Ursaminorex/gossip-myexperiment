...
//}
//
func originalGossiper(port int, round *int, colored map[int]int, ch chan int, csvWriter *csv.Writer) {
	ip := net.ParseIP(localhost)
	listen, err := net.ListenUDP("udp", &net.UDPAddr{
		IP:   ip,
		Port: port,
	...
	for {
		...
		go func(msg Message) {
			mutex.Lock()
			fmt.Printf("Data=%s, Round=%d, Path=%s, updnums=%d, roundnums=%d, coverage=%.2f%%\n", msg.Data, msg.Round, msg.Path, udpNums, roundNums, float32(len(colored))/float32(cfg.Count/100))
			udpNums++
			roundNums++
			colored[port]++ //记录节点收到消息的次数
			mutex.Unlock()
...
func asynOriginalGossiper(port int, colored map[int]int, ch chan int) {
	ip := net.ParseIP(localhost)
	listen, err := net.ListenUDP("udp", &net.UDPAddr{
		IP:   ip,
		Port: port,
	...
	for {
		...
		go func(msg Message) {
			mutex.Lock()
			fmt.Printf("Data=%s, Path=%s, updnums=%d, coverage=%.2f%%\n", msg.Data, msg.Path, udpNums, float32(len(colored))/float32(cfg.Count/100))
			udpNums++
			colored[port]++ //记录节点收到消息的次数
			mutex.Unlock()
