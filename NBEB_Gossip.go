...
func NBEBGossiper2(port int, round *int, isGossipList, changePList, hasPushList map[int]bool, pList, colored map[int]int, ch chan int) {
		...
		if isFirst {
			...
			go func(msg Message) {
				for { //阻塞等待下一轮屏障刷新
					...
					if port == cfg.Firstnode && randNeighbor == port+cfg.Count-1 {
						hasPushList[port] = true
						fmt.Println(port, " push to ", randNeighbor)
					} else if randNeighbor == port-1 {
						hasPushList[port] = true
						fmt.Println(port, " push to ", randNeighbor)
					}
