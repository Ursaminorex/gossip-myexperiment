...
func BEBG(port int, round *int, isGossipList, changePList map[int]bool, pList, colored map[int]int, ch chan int, csvWriter *csv.Writer) {
...
func MBEBG(port int, round *int, isGossipList, changePList map[int]bool, pList, colored map[int]int, ch chan int, csvWriter *csv.Writer) {
	ip := net.ParseIP(localhost)
	listen, err := net.ListenUDP("udp", &net.UDPAddr{
		IP:   ip,
		Port: port,
...
func BEBGossiper(port int, round, notGossipSum *int, colored map[int]int, ch chan int, csvWriter *csv.Writer) {
	ip := net.ParseIP(localhost)
	listen, err := net.ListenUDP("udp", &net.UDPAddr{
		IP:   ip,
		Port: port,
